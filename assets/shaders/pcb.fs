...
//
// original Shader made by: stupxd
// (modified)
...
void mainImage( out vec4 fragColor, in vec2 uv, in vec2 duv, in Image texture, in float sprite_width, in float sprite_height ) {
    ...
    pcbcol.g *= abs(1 + (cos((tilt.y * 1.5) + pcb.g) * 0.5));
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float newX = sprite_pos_x + sprite_width + sprite_pos_x;
    ...
    float newY = sprite_pos_y + sprite_height + sprite_pos_y;
    ...
    // required
    mainImage( colour, uv, uvq, texture, sprite_width, sprite_height );
    return dissolve_mask(colour, texture_coords, uv);
}
